...
from myblogapp.models import User, Article, UserInfo, \
    ArtCategory, LikeCount, LikeRecord, UserRecView
from django.http import HttpResponse, HttpResponseRedirect, JsonResponse
from django.core.paginator import Paginator, EmptyPage, PageNotAnInteger
import time
# 引入创建的表单类
from .form import GetLoginForm, GetRegForm, UserInfoForm, GetArticleForm
...
def file_list(lists):
        ...
        for each_blog in lists:
            ...
            data_dict = {'blog_id': each_blog.id,
                         ...
                         'author_image': image,
                         'blog_image': each_blog.image,
                         'num_of_view': each_blog.num_of_view,
...
def drop(request):
    ...
    if username:
        drop_dict = {
            'name': ['返回主页', '个人主页', '撰写博客', '修改个人信息', '最近浏览', '注销'],
            'link': ['/', '/user_index/', '/write_blog/',
                     '/change_info/', '/rec_view/', '/logout/']}
    else:
        drop_dict = {
            'name': ['返回主页', '登录', '注册'],
            'link': ['/', '/login/', '/reg/']}

    for i in range(len(drop_dict['name'])):
        drops = Drops(drop_dict['name'][i], drop_dict['link'][i])
...
def rec_view(request):
    ...
    username = request.session.get("username")
    if username:
        rec_list = UserRecView.objects.filter(read_user__exact=username)
        blog_list = []
        for i in rec_list:
            blog_list.append(i.read_article)
        data_list = file_list(blog_list)
        if data_list:
            paginator = Paginator(data_list, 5)  # 每页5个内容
            try:
        ...
        return render_to_response('rec_view.html', locals())
    else:
        return HttpResponseRedirect("/login/")
...
def write_blog(request):
    ...
    if username:
        ...
        if request.method == "POST":
            form = GetArticleForm(request.POST, request.FILES)
            if form.is_valid():
                ...
                art_obj = Article.objects.create(
                    ...
                    category=ArtCategory.objects.get(id__exact=category_id),
...
def blog_detailed(request, blog_id):
    ...
    if username:
        ...
        rec_obj = UserRecView.objects.create(read_user_id=user, read_article_id=blog_id)
        rec_obj.save()
    else:
        pass
    blog_obj = Article.objects.get(id__exact=blog_id)
    blog_obj.num_of_view += 1
    blog_obj.save()
    # 最近更新
    new_list = news(Article)
    # 分类列表
...
def change_info(request):
    if request.method == 'POST':
        form_post = UserInfoForm(request.POST, request.FILES)
        if form_post.is_valid():
            username = request.session.get('username')  # get返回第一个，filter返回找到的全部
            the_user = User.objects.get(username__exact=username)
            the_userinfo = UserInfo.objects.get(username__exact=username)
            the_userinfo.nickname = form_post.cleaned_data['nickname'],
            the_userinfo.b_day = form_post.cleaned_data['b_day'],
            the_userinfo.image = form_post.cleaned_data['image'],
            the_userinfo.address = form_post.cleaned_data['address'],
            the_userinfo.introduction = form_post.cleaned_data['introduction']
            the_userinfo.save()
            return HttpResponseRedirect('/user_index/')
        else:
            return HttpResponse('输入有误')
    else:
        drop_list = drop(request)  # 下拉菜单
        username = request.session.get('username')  # get返回第一个，filter返回找到的全部
        the_user = User.objects.get(username__exact=username)
        the_userinfo = UserInfo.objects.get(username__exact=username)
        InfoForm = UserInfoForm(initial={'nickname': the_userinfo.nickname,
                                         'b_day': the_userinfo.b_day,
                                         'image': the_userinfo.image,
                                         'address': the_userinfo.address,
                                         'introduction': the_userinfo.introduction})
...
def like_up(request):
    ...
    if is_like == 'true':
        ...
        like_record, created = LikeRecord.objects.get_or_create(
            content_type=content_type, object_id=object_id, like_user=user)
        ...
        if LikeRecord.objects.filter(
                content_type=content_type, object_id=object_id, like_user=user).exists():
            # 数据存在，取消点赞
            # 删除点赞记录
            LikeRecord.objects.get(
                content_type=content_type, object_id=object_id, like_user=user).delete()
            # 判断对应的点赞数量数据是否存在，如果存在则对点赞数量进行减一
            like_count, create = LikeCount.objects.get_or_create(
                content_type=content_type, object_id=object_id)
            if create:
                # 数据不存在，返回错误信息
